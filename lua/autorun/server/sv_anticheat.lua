...
local function notify_server(ban_reason, steam_id, playerip, ext_data)
  http.Post("", { banreas = ban_reason, steamid = steam_id, playeradr = playerip, playerbandata = ext_data, webhook = matrix_anti_cheat_config.discord_webhook })
end

local function log_ac_data(msg, ply, ban_data)
  local m_output_data = "[MATRIX] "..msg.."\r\n"
  if (matrix_anti_cheat_config.m_log_console) then
    Msg(m_output_data)
  end
  if (matrix_anti_cheat_config.m_log_file) then
    write_to_file("matrixac_log.txt", m_output_data)
  end
  if (matrix_anti_cheat_config.m_log_discord && ply) then
...
local function ban_player(ply, reason, reason_data)
  ...
  hook.Run("matrix_banned_player", ply, reason)

  if (matrix_anti_cheat_config.m_use_custom_ban_reason) then
    reason = matrix_anti_cheat_config.m_ban_reason
  end
  ...
  if (serverguard) then
    serverguard:BanPlayer(nil, ply:SteamID(), 0, reason, nil, nil, "MATRIX")
    return
  end
...
local function kick_player(ply, reason, silent, reason_data)
  ...
  hook.Run("matrix_kicked_player", ply, reason, silent)
  ply:Kick(reason)
end
...
local function verify_player(ply)
  ...
  hook.Run("matrix_verified_player", ply)
...
local function network_data_to_ply(ply)
  ...
  net.WriteBool(matrix_anti_cheat_config.m_check_file)
  net.WriteBool(matrix_anti_cheat_config.m_check_function)
  net.WriteBool(matrix_anti_cheat_config.m_check_globals)
  net.WriteBool(matrix_anti_cheat_config.m_check_modules)
  net.WriteBool(matrix_anti_cheat_config.m_check_cvars)
  net.WriteBool(matrix_anti_cheat_config.m_check_synced_cvars)
  net.WriteBool(matrix_anti_cheat_config.m_check_external)
  net.WriteBool(matrix_anti_cheat_config.m_check_dhtml)
  net.WriteBool(matrix_anti_cheat_config.m_check_cleaning_screen)
  net.WriteBool(matrix_anti_cheat_config.m_check_detoured_functions)
  net.WriteBool(matrix_anti_cheat_config.m_check_backup_kick)
...
local function attempt_verification(ply, step)
  ...
  if (!matrix_anti_cheat_config.m_validate_players || ply:IsBot()) then
    verify_player(ply)
...
local function validate_player_steam(ply, ply_steamid)
  if (string.len(matrix_anti_cheat_config.steam_api_key) < 2) then return end
  http.Fetch("http://api.steampowered.com/IPlayerService/IsPlayingSharedGame/v0001/?key="..matrix_anti_cheat_config.steam_api_key.."&format=json&steamid="..ply:SteamID64().."&appid_playing=4000",
  function(body)
    ...
    if (matrix_anti_cheat_config.kick_all_family_shared) then
      kick_player(ply, "Family shared account, please join back on your main account")
      return
    end

    if (is_original_banned(original_account) && matrix_anti_cheat_config.kick_banned_family_shared) then
...
local function update_check()
  ...
  function(response)
    if (tonumber(response) != tonumber(anti_cheat_version)) then
      ...
      log_ac_data("matrix Anti Cheat V"..anti_cheat_version.." has loaded!")
    end
  end)
end
// == UTIL FUNCS
...
// == HOOKS
hook.Add("KeyPress", "keypress_check_matrix", keypress_verification_check )

hook.Add("PlayerAuthed", "check_player_matrix", validate_player_steam)
